...
- Inline metadata preview in the `SecurityIndicatorDot` popover (lookup pages). `ConstrainedDelegation` lists the first 3 target SPNs, `Rbcd` lists the first 3 allowed-principal SIDs, with a `+N more` truncation suffix when the list overflows. The operator no longer has to open `ComputerDetail` to see what the configuration permits. New `dot.metadataMore` i18n key translated to en/fr/de/it/es. Closes QA-14.3-003.
...
- Move `tooltipParamsFor` helper from `ComputerDetail` to `src/types/securityIndicators.ts` so `SecurityIndicatorDot` reuses the same metadata-to-i18n-placeholder mapping. Adding a future metadata-bearing indicator only requires extending one function. Closes QA-14.3-002.
