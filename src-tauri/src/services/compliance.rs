...
fn compute_framework_scores(results: &[CheckResult]) -> Vec<FrameworkScore> {
    FRAMEWORKS
        ...
        .map(|&std_name| {
            ...
            let with_findings = relevant_checks.iter().filter(|r| r.finding_count > 0).count();
...
pub fn export_framework_report(scan: &ComplianceScanResult, framework: &str) -> String {
    ...
    let score_color = if score >= 80 { "#2e7d32" } else if score >= 50 { "#e65100" } else { "#c62828" };
    ...
    html.push_str(&format!(
        ...
        he(&scan.scanned_at), he(&scan.generator),
    ));
    html.push_str("</div>\n");
    ...
    html.push_str(&format!(
        ...
        score_color, score,
        relevant_checks.len(),
        relevant_checks.iter().filter(|c| c.finding_count > 0).count(),
    ));

    // TOC
    html.push_str("<div class=\"section\">\n<h2>Table of Contents</h2>\n<ol>\n");
    for (i, check) in relevant_checks.iter().enumerate() {
        let ctrl = check.frameworks.iter()
            .find(|f| f.standard == framework)
    ...
    for (i, check) in relevant_checks.iter().enumerate() {
        ...
        html.push_str(&format!(
            ...
            he(ctrl), sev_class, he(&check.severity),
        ));
        html.push_str(&format!("<p class=\"desc\">{}</p>\n", he(&check.description)));
        html.push_str(&format!("<div class=\"count\">{} findings</div>\n", check.finding_count));
        ...
        html.push_str(&format!("<div class=\"remediation\">{}</div>\n", he(&check.remediation)));
        html.push_str("</div>\n");
    }

    html.push_str(&format!(
        "<div class=\"footer\">Generated by DSPanel - {} - {}</div>\n",
        he(framework), he(&scan.scanned_at),
    ));
    html.push_str("</div>\n</body>\n</html>");
    ...
    match attr_name {
        ...
        "userAccountControl" => {
            if let Ok(uac) = value.parse::<u32>() {
                ...
                if uac & 0x0080 != 0 { flags.push("ReversibleEncryption"); }
                if uac & 0x200000 != 0 { flags.push("TrustedForDelegation"); }
                if flags.is_empty() { "Normal".to_string() } else { flags.join(", ") }
            } else {
                value.to_string()
            }
        }
...
mod tests {
    ...
    fn all_checks_have_remediation() {
        for c in builtin_checks() {
            assert!(!c.remediation.is_empty(), "Check {} has no remediation", c.id);
        }
    }
    ...
    fn all_nine_frameworks_covered() {
        ...
        for &fw in FRAMEWORKS {
            let covered = checks.iter().any(|c| c.frameworks.iter().any(|f| f.standard == fw));
    ...
    fn critical_checks_map_to_all_frameworks() {
        let checks = builtin_checks();
        let critical: Vec<&ComplianceCheck> = checks.iter().filter(|c| c.severity == "Critical").collect();
        assert!(critical.len() >= 2);
        for c in critical {
            assert!(c.frameworks.len() >= 7, "Critical check {} should map to most frameworks", c.id);
        }
    }
    ...
    fn compute_score_with_findings() {
        let results = vec![
            CheckResult {
                check_id: "a".into(), title: "".into(), description: "".into(),
                severity: "Critical".into(), finding_count: 2,
                headers: vec![], rows: vec![vec![], vec![]],
                frameworks: vec![fm("GDPR", "x")], remediation: "".into(),
            },
            CheckResult {
                check_id: "b".into(), title: "".into(), description: "".into(),
                severity: "High".into(), finding_count: 1,
                headers: vec![], rows: vec![vec![]],
                frameworks: vec![fm("GDPR", "y")], remediation: "".into(),
            },
        ];
    ...
    fn framework_scores_computed() {
        let results = vec![CheckResult {
            check_id: "test".into(), title: "".into(), description: "".into(),
            severity: "High".into(), finding_count: 3,
            headers: vec![], rows: vec![vec![], vec![], vec![]],
    ...
    fn format_uac_flags() {
        ...
        assert_eq!(format_ad_attribute("userAccountControl", "66048"), "PwdNeverExpires");
    }
