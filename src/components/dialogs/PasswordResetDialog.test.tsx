...
describe("PasswordResetDialog", () => {
  ...
  it("calls generate_password on generate click", async () => {
    ...
    expect(screen.getByTestId("generated-password")).toHaveTextContent("GeneratedP@ss1");
  });
  ...
  it("shows result view with copyable password on success", async () => {
    ...
    expect(screen.getByTestId("result-password")).toHaveTextContent("Success123!");
  ...
  it("shows error message on reset failure", async () => {
    mockInvoke.mockImplementation(((cmd: string) => {
      ...
      if (cmd === "reset_password") return Promise.reject('{"userMessage":"Policy violation"}');
      return Promise.resolve(undefined);
    }) as typeof invoke);
    ...
    expect(screen.getByTestId("error-message")).toHaveTextContent("Policy violation");
  });
